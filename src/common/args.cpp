...
std::list<SectionInfo> ArgsManager::GetUnrecognizedSections() const
{
    ...
    static const std::set<std::string> available_sections{
        ...
        ChainTypeToString(ChainType::OPNET_TESTNET),
...
std::variant<ChainType, std::string> ArgsManager::GetChainArg() const
{
    ...
    const bool fOpnetTestnet = get_net("-opnet-testnet");
    const auto chain_arg = GetArg("-chain");

    if ((int)chain_arg.has_value() + (int)fRegTest + (int)fSigNet + (int)fTestNet + (int)fTestNet4 + (int)fOpnetTestnet > 1) {
        throw std::runtime_error("Invalid combination of -regtest, -signet, -testnet, -testnet4, -opnet-testnet and -chain. Can use at most one.");
    }
    if (chain_arg) {
    ...
    if (fOpnetTestnet) return ChainType::OPNET_TESTNET;
